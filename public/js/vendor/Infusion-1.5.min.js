 ...
 *  jquery.ui.mouse.js
 */
(function($){$.support.touch="ontouchend" in document;if(!$.support.touch){return }var mouseProto=$.ui.mouse.prototype,_mouseInit=mouseProto._mouseInit,touchHandled;function simulateMouseEvent(event,simulatedType){if(event.originalEvent.touches.length>1){return }event.preventDefault();var touch=event.originalEvent.changedTouches[0],simulatedEvent=document.createEvent("MouseEvents");simulatedEvent.initMouseEvent(simulatedType,true,true,window,1,touch.screenX,touch.screenY,touch.clientX,touch.clientY,false,false,false,false,0,null);event.target.dispatchEvent(simulatedEvent)}mouseProto._touchStart=function(event){var self=this;if(touchHandled||!self._mouseCapture(event.originalEvent.changedTouches[0])){return }touchHandled=true;self._touchMoved=false;simulateMouseEvent(event,"mouseover");simulateMouseEvent(event,"mousemove");simulateMouseEvent(event,"mousedown")};mouseProto._touchMove=function(event){if(!touchHandled){return }this._touchMoved=true;simulateMouseEvent(event,"mousemove")};mouseProto._touchEnd=function(event){if(!touchHandled){return }simulateMouseEvent(event,"mouseup");simulateMouseEvent(event,"mouseout");if(!this._touchMoved){simulateMouseEvent(event,"click")}touchHandled=false};mouseProto._mouseInit=function(){var self=this;self.element.bind("touchstart",$.proxy(self,"_touchStart")).bind("touchmove",$.proxy(self,"_touchMove")).bind("touchend",$.proxy(self,"_touchEnd"));_mouseInit.call(self)}})(jQuery);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.registerNamespace("fluid.url");fluid.url.generateDepth=function(depth){return fluid.generate(depth,"../").join("")};fluid.url.parsePathInfo=function(pathInfo){var togo={};var segs=pathInfo.split("/");if(segs.length>0){var top=segs.length-1;var dotpos=segs[top].indexOf(".");if(dotpos!==-1){togo.extension=segs[top].substring(dotpos+1);segs[top]=segs[top].substring(0,dotpos)}}togo.pathInfo=segs;return togo};fluid.url.parsePathInfoTrim=function(pathInfo){var togo=fluid.url.parsePathInfo(pathInfo);if(togo.pathInfo[togo.pathInfo.length-1]===""){togo.pathInfo.length--}return togo};fluid.url.collapseSegs=function(segs,from,to){var togo="";if(from===undefined){from=0}if(to===undefined){to=segs.length}for(var i=from;i<to-1;++i){togo+=segs[i]+"/"}if(to>from){togo+=segs[to-1]}return togo};fluid.url.makeRelPath=function(parsed,index){var togo=fluid.kettle.collapseSegs(parsed.pathInfo,index);if(parsed.extension){togo+="."+parsed.extension}return togo};fluid.url.cononocolosePath=function(pathInfo){var consume=0;for(var i=0;i<pathInfo.length;++i){if(pathInfo[i]===".."){++consume}else{if(consume!==0){pathInfo.splice(i-consume*2,consume*2);i-=consume*2;consume=0}}}return pathInfo};fluid.url.parseUri=function(str){var o=fluid.url.parseUri.options,m=o.parser[o.strictMode?"strict":"loose"].exec(str),uri={},i=14;while(i--){uri[o.key[i]]=m[i]||""}uri[o.q.name]={};uri[o.key[12]].replace(o.q.parser,function($0,$1,$2){if($1){uri[o.q.name][$1]=$2}});return uri};fluid.url.parseUri.options={strictMode:true,key:["source","protocol","authority","userInfo","user","password","host","port","relative","path","directory","file","query","anchor"],q:{name:"queryKey",parser:/(?:^|&)([^&=]*)=?([^&]*)/g},parser:{strict:/^(?:([^:\/?#]+):)?(?:\/\/((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:\/?#]*)(?::(\d*))?))?((((?:[^?#\/]*\/)*)([^?#]*))(?:\?([^#]*))?(?:#(.*))?)/,loose:/^(?:(?![^:@]+:[^:@\/]*@)([^:\/?#.]+):)?(?:\/\/)?((?:(([^:@]*)(?::([^:@]*))?)?@)?([^:\/?#]*)(?::(\d*))?)(((\/(?:[^?#](?![^?#\/]*\.[^?#\/.]+(?:[?#]|$)))*\/?)?([^?#\/]*))(?:\?([^#]*))?(?:#(.*))?)/}};fluid.url.parseSegs=function(url){var parsed=fluid.url.parseUri(url);var parsedSegs=fluid.url.parsePathInfoTrim(parsed.directory);return parsedSegs.pathInfo};fluid.url.isAbsoluteUrl=function(url){var parseRel=fluid.url.parseUri(url);return(parseRel.host||parseRel.protocol||parseRel.directory.charAt(0)==="/")};fluid.url.computeRelativePrefix=function(outerLocation,iframeLocation,relPath){if(fluid.url.isAbsoluteUrl(relPath)){return relPath}var relSegs=fluid.url.parsePathInfo(relPath).pathInfo;var parsedOuter=fluid.url.parseSegs(outerLocation);var parsedRel=parsedOuter.concat(relSegs);fluid.url.cononocolosePath(parsedRel);var parsedInner=fluid.url.parseSegs(iframeLocation);var seg=0;for(;seg<parsedRel.length;++seg){if(parsedRel[seg]!=parsedInner[seg]){break}}var excess=parsedInner.length-seg;var back=fluid.url.generateDepth(excess);var front=fluid.url.collapseSegs(parsedRel,seg);return back+front}})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.uiOptions.dataSource",{gradeNames:["fluid.littleComponent"],invokers:{get:"fluid.uiOptions.dataSource.get",set:"fluid.uiOptions.dataSource.set"}});fluid.defaults("fluid.cookieStore",{gradeNames:["fluid.uiOptions.dataSource","autoInit"],cookie:{name:"fluid-ui-settings",path:"/",expires:""}});fluid.demands("fluid.uiOptions.dataSource.get","fluid.cookieStore",{funcName:"fluid.cookieStore.get",args:"{that}.options.cookie.name"});fluid.demands("fluid.uiOptions.dataSource.set","fluid.cookieStore",{funcName:"fluid.cookieStore.set",args:["{arguments}.0","{that}.options.cookie"]});fluid.cookieStore.get=function(cookieName){var cookie=document.cookie;if(cookie.length<=0){return }var cookiePrefix=cookieName+"=";var startIndex=cookie.indexOf(cookiePrefix);if(startIndex<0){return }startIndex=startIndex+cookiePrefix.length;var endIndex=cookie.indexOf(";",startIndex);if(endIndex<startIndex){endIndex=cookie.length}var retObj=JSON.parse(decodeURIComponent(cookie.substring(startIndex,endIndex)));return retObj};fluid.cookieStore.assembleCookie=function(cookieOptions){var cookieStr=cookieOptions.name+"="+cookieOptions.data;if(cookieOptions.expires){cookieStr+="; expires="+cookieOptions.expires}if(cookieOptions.path){cookieStr+="; path="+cookieOptions.path}return cookieStr};fluid.cookieStore.set=function(settings,cookieOptions){cookieOptions.data=encodeURIComponent(JSON.stringify(settings));document.cookie=fluid.cookieStore.assembleCookie(cookieOptions)};fluid.defaults("fluid.tempStore",{gradeNames:["fluid.uiOptions.dataSource","fluid.modelComponent","autoInit"]});fluid.demands("fluid.uiOptions.dataSource.get","fluid.tempStore",{funcName:"fluid.identity",args:"{that}.model"});fluid.demands("fluid.uiOptions.dataSource.set","fluid.tempStore",{funcName:"fluid.tempStore.set",args:["{arguments}.0","{that}.applier"]});fluid.tempStore.set=function(settings,applier){applier.requestChange("",settings)};fluid.defaults("fluid.globalSettingsStore",{gradeNames:["autoInit","fluid.littleComponent"],components:{settingsStore:{type:"fluid.uiOptions.store"}}});fluid.globalSettingsStore.finalInit=function(that){fluid.staticEnvironment.settingsStore=that.settingsStore};fluid.demands("fluid.uiOptions.store",["fluid.globalSettingsStore"],{funcName:"fluid.cookieStore"})})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.textfieldSlider",{gradeNames:["fluid.viewComponent","autoInit"],components:{textfield:{type:"fluid.textfieldSlider.textfield",container:"{textfieldSlider}.dom.textfield",options:{model:"{textfieldSlider}.model",range:"{textfieldSlider}.options.range",applier:"{textfieldSlider}.applier"}},slider:{type:"fluid.textfieldSlider.slider",container:"{textfieldSlider}.dom.slider",options:{model:"{textfieldSlider}.model",range:"{textfieldSlider}.options.range",applier:"{textfieldSlider}.applier",sliderOptions:"{textfieldSlider}.options.sliderOptions"}}},selectors:{textfield:".flc-textfieldSlider-field",slider:".flc-textfieldSlider-slider"},events:{modelChanged:null,afterRender:null},listeners:{modelChanged:"{that}.refreshView"},model:{value:null},range:{min:0,max:100},sliderOptions:{orientation:"horizontal",step:1},invokers:{refreshView:{funcName:"fluid.textfieldSlider.refreshView",args:["{that}"]}},finalInitFunction:"fluid.textfieldSlider.finalInit",renderOnInit:true});fluid.textfieldSlider.finalInit=function(that){that.applier.modelChanged.addListener("value",function(newModel){that.events.modelChanged.fire(newModel.value)});if(that.options.renderOnInit){that.refreshView()}};fluid.textfieldSlider.refreshView=function(that){that.textfield.container.val(that.model.value);that.events.afterRender.fire(that)};fluid.defaults("fluid.textfieldSlider.textfield",{gradeNames:["fluid.viewComponent","autoInit"],listeners:{onCreate:{listener:"fluid.textfieldSlider.textfield.init",args:"{that}"}},range:{}});fluid.textfieldSlider.validateValue=function(model,range,changeRequest){var oldValue=model.value;var newValue=changeRequest.value;var isValidNum=!isNaN(parseInt(newValue,10));if(isValidNum){if(newValue<range.min){newValue=range.min}else{if(newValue>range.max){newValue=range.max}}changeRequest.value=newValue}else{changeRequest.value=oldValue}};fluid.textfieldSlider.textfield.init=function(that){that.applier.guards.addListener({path:"value",transactional:true},function(model,changeRequest){fluid.textfieldSlider.validateValue(model,that.options.range,changeRequest)});that.container.change(function(source){that.applier.requestChange("value",source.target.value)})};fluid.defaults("fluid.textfieldSlider.slider",{gradeNames:["fluid.viewComponent","autoInit"],selectors:{thumb:".ui-slider-handle"},events:{modelChanged:null},listeners:{onCreate:{listener:"fluid.textfieldSlider.slider.init",args:"{that}"}},range:{}});var initSliderAria=function(thumb,opts){var ariaDefaults={role:"slider","aria-valuenow":opts.value,"aria-valuemin":opts.min,"aria-valuemax":opts.max};thumb.attr(ariaDefaults)};fluid.textfieldSlider.slider.init=function(that){var sliderOptions=$.extend(true,{},that.options.sliderOptions,that.model,that.options.range);that.slider=that.container.slider(sliderOptions);initSliderAria(that.locate("thumb"),sliderOptions);that.setSliderValue=function(value){that.slider.slider("value",value)};that.setSliderAria=function(value){that.locate("thumb").attr("aria-valuenow",value)};that.slider.bind("slide",function(e,ui){that.applier.requestChange("value",ui.value)});that.applier.modelChanged.addListener("value",function(newModel){that.setSliderValue(newModel.value);that.setSliderAria(newModel.value);that.events.modelChanged.fire(newModel.value)})}})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.uiOptions.rootModel",{gradeNames:["fluid.littleComponent","autoInit"],members:{rootModel:{}}});fluid.defaults("fluid.uiEnhancer",{gradeNames:["fluid.viewComponent","autoInit"],invokers:{updateModel:{funcName:"fluid.uiEnhancer.updateModel",args:["{arguments}.0","{uiEnhancer}.applier"]}}});fluid.uiEnhancer.updateModel=function(newModel,applier){applier.requestChange("",newModel)};fluid.defaults("fluid.pageEnhancer",{gradeNames:["fluid.eventedComponent","fluid.originalEnhancerOptions","fluid.uiOptions.rootModel","fluid.uiOptions.settingsGetter","autoInit"],components:{uiEnhancer:{type:"fluid.uiEnhancer",container:"body"}},distributeOptions:{source:"{that}.options.uiEnhancer",target:"{that > uiEnhancer}.options"},invokers:{init:{funcName:"fluid.pageEnhancer.init",args:"{that}"}},listeners:{onCreate:[{listener:"{that}.init"}]}});fluid.pageEnhancer.init=function(that){that.options.originalUserOptions=$.extend(true,that.uiEnhancer.options,fluid.copy(that.options.uiEnhancer));fluid.staticEnvironment.originalEnhancerOptions=that;that.uiEnhancer.updateModel(that.getSettings());fluid.staticEnvironment.uiEnhancer=that.uiEnhancer}})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.uiOptions.uiOptionsLoader",{gradeNames:["fluid.viewComponent","autoInit"],components:{uiOptions:{priority:"last",type:"fluid.uiOptions",createOnEvent:"onCreateUIOptionsReady"},templateLoader:{type:"fluid.uiOptions.resourceLoader",options:{events:{onResourcesLoaded:"{uiOptionsLoader}.events.onUIOTemplatesLoaded"}}},messageLoader:{type:"fluid.uiOptions.resourceLoader",options:{events:{onResourcesLoaded:"{uiOptionsLoader}.events.onUIOMessagesLoaded"}}}},events:{onUIOTemplatesLoaded:null,onUIOMessagesLoaded:null,onMsgBundleReady:null,onCreateUIOptionsReady:{events:{templateLoaded:"onUIOTemplatesLoaded",msgBundleReady:"onMsgBundleReady"}}},listeners:{onUIOMessagesLoaded:{funcName:"fluid.uiOptions.uiOptionsLoader.createMsgBundle",args:["{arguments}.0","{that}"]}},distributeOptions:[{source:"{that}.options.templateLoader",removeSource:true,target:"{that > templateLoader}.options"},{source:"{that}.options.messageLoader",removeSource:true,target:"{that > messageLoader}.options"},{source:"{that}.options.templatePrefix",target:"{that > templateLoader > resourcePath}.options.value"},{source:"{that}.options.messagePrefix",target:"{that > messageLoader > resourcePath}.options.value"},{source:"{that}.options.uiOptions",removeSource:true,target:"{that > uiOptions}.options"}]});fluid.uiOptions.uiOptionsLoader.createMsgBundle=function(messageResources,that){var completeMessage;fluid.each(messageResources,function(oneResource){var message=JSON.parse(oneResource.resourceText);completeMessage=$.extend({},completeMessage,message)});var parentResolver=fluid.messageResolver({messageBase:completeMessage});that.msgBundle=fluid.messageResolver({messageBase:{},parents:[parentResolver]});that.events.onMsgBundleReady.fire()};fluid.defaults("fluid.uiOptions.transformDefaultPanelsOptions",{gradeNames:["fluid.viewComponent","autoInit"],distributeOptions:[{source:"{that}.options.textSize",removeSource:true,target:"{that textSize}.options"},{source:"{that}.options.lineSpace",removeSource:true,target:"{that lineSpace}.options"},{source:"{that}.options.textFont",removeSource:true,target:"{that textFont}.options"},{source:"{that}.options.contrast",removeSource:true,target:"{that contrast}.options"},{source:"{that}.options.layoutControls",removeSource:true,target:"{that layoutControls}.options"},{source:"{that}.options.linksControls",removeSource:true,target:"{that linksControls}.options"}]});fluid.defaults("fluid.uiOptions.resourceLoader",{gradeNames:["fluid.eventedComponent","autoInit"],listeners:{onCreate:{listener:"fluid.uiOptions.resourceLoader.loadTemplates",args:["{that}",{expander:{func:"{that}.resolveTemplates"}}]}},templates:{},components:{resourcePath:{type:"fluid.uiOptions.resourcePath"}},invokers:{transformURL:{funcName:"fluid.stringTemplate",args:["{arguments}.0",{"prefix/":"{that}.resourcePath.options.value"}]},resolveTemplates:{funcName:"fluid.uiOptions.resourceLoader.resolveTemplates",args:"{that}"}},events:{onResourcesLoaded:null}});fluid.uiOptions.resourceLoader.resolveTemplates=function(that){var mapped=fluid.transform(that.options.templates,that.transformURL);return fluid.transform(mapped,function(url){return{url:url,forceCache:true}})};fluid.uiOptions.resourceLoader.loadTemplates=function(that,resources){delete resources.expander;fluid.fetchResources(resources,function(){that.resources=resources;that.events.onResourcesLoaded.fire(resources)})};fluid.defaults("fluid.uiOptions.resourcePath",{gradeNames:["fluid.littleComponent","autoInit"],value:"../html/"});fluid.defaults("fluid.uiOptions.settingsGetter",{gradeNames:["fluid.littleComponent","autoInit"],members:{getSettings:"{settingsStore}.get"}});fluid.defaults("fluid.uiOptions.settingsSetter",{gradeNames:["fluid.littleComponent","autoInit"],invokers:{setSettings:{funcName:"fluid.uiOptions.settingsSetter.setSettings",args:["{arguments}.0","{settingsStore}.set"]}}});fluid.uiOptions.settingsSetter.setSettings=function(model,set){var userSettings=fluid.copy(model);set(userSettings)};fluid.defaults("fluid.uiOptions.uiEnhancerRelay",{gradeNames:["autoInit","fluid.eventedComponent"],listeners:{onCreate:"{that}.addListener",onDestroy:"{that}.removeListener"},events:{updateEnhancerModel:"{fluid.uiOptions}.events.onUpdateEnhancerModel"},invokers:{addListener:{funcName:"fluid.uiOptions.uiEnhancerRelay.addListener",args:["{that}.events.updateEnhancerModel","{that}.updateEnhancerModel"]},removeListener:{funcName:"fluid.uiOptions.uiEnhancerRelay.removeListener",args:["{that}.events.updateEnhancerModel","{that}.updateEnhancerModel"]},updateEnhancerModel:{funcName:"fluid.uiOptions.uiEnhancerRelay.updateEnhancerModel",args:["{uiEnhancer}","{fluid.uiOptions}.model"]}}});fluid.uiOptions.uiEnhancerRelay.addListener=function(modelChanged,listener){modelChanged.addListener(listener)};fluid.uiOptions.uiEnhancerRelay.removeListener=function(modelChanged,listener){modelChanged.removeListener(listener)};fluid.uiOptions.uiEnhancerRelay.updateEnhancerModel=function(uiEnhancer,newModel){uiEnhancer.updateModel(newModel)};fluid.defaults("fluid.uiOptions",{gradeNames:["fluid.viewComponent","fluid.uiOptions.settingsGetter","fluid.uiOptions.settingsSetter","fluid.uiOptions.rootModel","autoInit"],invokers:{updateModel:{funcName:"fluid.fireSourcedChange",args:["{that}.applier","","{arguments}.0","{arguments}.1"]},fetch:{funcName:"fluid.uiOptions.fetch",args:["{that}"]},applyChanges:{funcName:"fluid.uiOptions.applyChanges",args:["{that}"]},save:{funcName:"fluid.uiOptions.save",args:["{that}"]},saveAndApply:{funcName:"fluid.uiOptions.saveAndApply",args:["{that}"]},reset:{funcName:"fluid.uiOptions.reset",args:["{that}"]},cancel:{funcName:"fluid.uiOptions.cancel",args:["{that}"]}},selectors:{cancel:".flc-uiOptions-cancel",reset:".flc-uiOptions-reset",save:".flc-uiOptions-save",previewFrame:".flc-uiOptions-preview-frame"},events:{onSave:null,onCancel:null,onReset:null,onAutoSave:null,modelChanged:null,onUIOptionsRefresh:null,onUpdateEnhancerModel:null,onUIOptionsMarkupReady:null,onReady:null},listeners:{onCreate:"fluid.uiOptions.init",onAutoSave:"{that}.save"},resources:{template:"{templateLoader}.resources.uiOptions"},autoSave:false});fluid.uiOptions.applyChanges=function(that){that.events.onUpdateEnhancerModel.fire()};fluid.uiOptions.fetch=function(that){var completeModel=that.getSettings();completeModel=$.extend(true,{},that.rootModel,completeModel);that.updateModel(completeModel,"settingsStore");that.events.onUIOptionsRefresh.fire();that.applyChanges()};fluid.uiOptions.save=function(that){that.events.onSave.fire(that.model);var savedSelections=fluid.copy(that.model);that.setSettings(savedSelections)};fluid.uiOptions.saveAndApply=function(that){that.save();that.events.onUIOptionsRefresh.fire();that.applyChanges()};fluid.uiOptions.reset=function(that){that.updateModel(fluid.copy(that.rootModel));that.events.onUIOptionsRefresh.fire();that.events.onReset.fire(that)};fluid.uiOptions.cancel=function(that){that.events.onCancel.fire();that.fetch()};fluid.uiOptions.finishInit=function(that){var bindHandlers=function(that){var saveButton=that.locate("save");if(saveButton.length>0){saveButton.click(that.saveAndApply);var form=fluid.findForm(saveButton);$(form).submit(function(){that.saveAndApply()})}that.locate("reset").click(that.reset);that.locate("cancel").click(that.cancel)};that.container.append(that.options.resources.template.resourceText);bindHandlers(that);that.events.onUIOptionsMarkupReady.fire(that);that.fetch();that.events.onReady.fire(that)};fluid.uiOptions.init=function(that){that.applier.modelChanged.addListener("",function(newModel,oldModel,changeRequest){that.events.modelChanged.fire(newModel,oldModel,changeRequest[0].source);if(that.options.autoSave){that.events.onAutoSave.fire()}});setTimeout(function(){fluid.uiOptions.finishInit(that)},1)};fluid.defaults("fluid.uiOptions.preview",{gradeNames:["fluid.viewComponent","autoInit"],components:{enhancer:{type:"fluid.uiEnhancer",container:"{preview}.enhancerContainer",createOnEvent:"onReady"},templateLoader:"{templateLoader}"},invokers:{updateModel:{funcName:"fluid.uiOptions.preview.updateModel",args:["{preview}","{uiOptions}.model"]}},finalInitFunction:"fluid.uiOptions.preview.finalInit",events:{onReady:null},listeners:{"{uiOptions}.events.modelChanged":"{that}.updateModel",onReady:"{that}.updateModel"},templateUrl:"%prefix/UIOptionsPreview.html"});fluid.uiOptions.preview.updateModel=function(that,model){setTimeout(function(){if(that.enhancer){that.enhancer.updateModel(model)}},0)};fluid.uiOptions.preview.finalInit=function(that){var templateUrl=that.templateLoader.transformURL(that.options.templateUrl);that.container.load(function(){that.enhancerContainer=$("body",that.container.contents());that.events.onReady.fire()});that.container.attr("src",templateUrl)}})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.uiOptions.panels",{gradeNames:["fluid.rendererComponent","fluid.uiOptions.modelRelay","autoInit"]});fluid.defaults("fluid.uiOptions.textfieldSlider",{gradeNames:["fluid.textfieldSlider","autoInit"],model:"{fluid.uiOptions.panels}.model",range:"{fluid.uiOptions.panels}.options.range",listeners:{modelChanged:{listener:"{fluid.uiOptions.panels}.applier.requestChange",args:["{that}.options.path","{arguments}.0"]}},path:"value",sliderOptions:"{fluid.uiOptions.panels}.options.sliderOptions"});fluid.uiOptions.panels.lookupMsg=function(messageResolver,prefix,values){var messages=[];fluid.each(values,function(value,key){var looked=messageResolver.lookup([prefix+"."+value]);messages.push(looked?looked.template:looked)});return messages};fluid.defaults("fluid.uiOptions.panels.textSize",{gradeNames:["fluid.uiOptions.panels","autoInit"],preferenceMap:{"fluid.uiOptions.textSize":{"model.value":"default","range.min":"minimum","range.max":"maximum"}},range:{min:1,max:2},selectors:{textSize:".flc-uiOptions-min-text-size",label:".flc-uiOptions-min-text-size-label",smallIcon:".flc-uiOptions-min-text-size-smallIcon",largeIcon:".flc-uiOptions-min-text-size-largeIcon",multiplier:".flc-uiOptions-multiplier"},protoTree:{label:{messagekey:"textSizeLabel"},smallIcon:{messagekey:"textSizeSmallIcon"},largeIcon:{messagekey:"textSizeLargeIcon"},multiplier:{messagekey:"multiplier"},textSize:{decorators:{type:"fluid",func:"fluid.uiOptions.textfieldSlider"}}},sliderOptions:{orientation:"horizontal",step:0.1,range:"min"}});fluid.defaults("fluid.uiOptions.panels.textFont",{gradeNames:["fluid.uiOptions.panels","autoInit"],preferenceMap:{"fluid.uiOptions.textFont":{"model.value":"default","controlValues.textFont":"enum"}},selectors:{textFont:".flc-uiOptions-text-font",label:".flc-uiOptions-text-font-label"},strings:{textFont:{expander:{func:"fluid.uiOptions.panels.lookupMsg",args:["{that}.options.parentBundle","textFont","{that}.options.controlValues.textFont"]}}},protoTree:{label:{messagekey:"textFontLabel"},textFont:{optionnames:"${{that}.options.strings.textFont}",optionlist:"${{that}.options.controlValues.textFont}",selection:"${value}",decorators:{type:"fluid",func:"fluid.uiOptions.selectDecorator",options:{styles:"{that}.options.classnameMap.textFont"}}}},classnameMap:null,controlValues:{textFont:["default","times","comic","arial","verdana"]}});fluid.defaults("fluid.uiOptions.panels.lineSpace",{gradeNames:["fluid.uiOptions.panels","autoInit"],preferenceMap:{"fluid.uiOptions.lineSpace":{"model.value":"default","range.min":"minimum","range.max":"maximum"}},range:{min:1,max:2},selectors:{lineSpace:".flc-uiOptions-line-space",label:".flc-uiOptions-line-space-label",narrowIcon:".flc-uiOptions-line-space-narrowIcon",wideIcon:".flc-uiOptions-line-space-wideIcon",multiplier:".flc-uiOptions-multiplier"},protoTree:{label:{messagekey:"lineSpaceLabel"},narrowIcon:{messagekey:"lineSpaceNarrowIcon"},wideIcon:{messagekey:"lineSpaceWideIcon"},multiplier:{messagekey:"multiplier"},lineSpace:{decorators:{type:"fluid",func:"fluid.uiOptions.textfieldSlider"}}},sliderOptions:{orientation:"horizontal",step:0.1,range:"min"}});fluid.defaults("fluid.uiOptions.panels.contrast",{gradeNames:["fluid.uiOptions.panels","autoInit"],preferenceMap:{"fluid.uiOptions.contrast":{"model.value":"default","controlValues.theme":"enum"}},listeners:{afterRender:"{that}.style"},selectors:{themeRow:".flc-uiOptions-themeRow",themeLabel:".flc-uiOptions-theme-label",themeInput:".flc-uiOptions-themeInput",label:".flc-uiOptions-contrast-label"},strings:{theme:{expander:{func:"fluid.uiOptions.panels.lookupMsg",args:["{that}.options.parentBundle","contrast","{that}.options.controlValues.theme"]}}},repeatingSelectors:["themeRow"],protoTree:{label:{messagekey:"contrastLabel"},expander:{type:"fluid.renderer.selection.inputs",rowID:"themeRow",labelID:"themeLabel",inputID:"themeInput",selectID:"theme-radio",tree:{optionnames:"${{that}.options.strings.theme}",optionlist:"${{that}.options.controlValues.theme}",selection:"${value}"}}},controlValues:{theme:["default","bw","wb","by","yb","lgdg"]},markup:{label:'<span class="fl-preview-A">A</span><span class="fl-hidden-accessible">%theme</span><div class="fl-crossout"></div>'},invokers:{style:{funcName:"fluid.uiOptions.panels.contrast.style",args:["{that}.dom.themeLabel","{that}.options.strings.theme","{that}.options.markup.label","{that}.options.controlValues.theme","{that}.options.classnameMap.theme"]}}});fluid.uiOptions.panels.contrast.style=function(labels,strings,markup,theme,style){fluid.each(labels,function(label,index){label=$(label);label.html(fluid.stringTemplate(markup,{theme:strings[index]}));label.addClass(style[theme[index]])})};fluid.defaults("fluid.uiOptions.panels.layoutControls",{gradeNames:["fluid.uiOptions.panels","autoInit"],preferenceMap:{"fluid.uiOptions.tableOfContents":{"model.toc":"default"}},selectors:{toc:".flc-uiOptions-toc",label:".flc-uiOptions-toc-label",choiceLabel:".flc-uiOptions-toc-choice-label"},protoTree:{label:{messagekey:"tocLabel"},choiceLabel:{messagekey:"tocChoiceLabel"},toc:"${toc}"}});fluid.defaults("fluid.uiOptions.panels.linksControls",{gradeNames:["fluid.uiOptions.panels","autoInit"],preferenceMap:{"fluid.uiOptions.emphasizeLinks":{"model.links":"default"},"fluid.uiOptions.inputsLarger":{"model.inputsLarger":"default"}},selectors:{links:".flc-uiOptions-links",inputsLarger:".flc-uiOptions-inputs-larger",label:".flc-uiOptions-links-label",linksChoiceLabel:".flc-uiOptions-links-choice-label",inputsChoiceLabel:".flc-uiOptions-links-inputs-choice-label"},protoTree:{label:{messagekey:"linksLabel"},linksChoiceLabel:{messagekey:"linksChoiceLabel"},inputsChoiceLabel:{messagekey:"inputsChoiceLabel"},links:"${links}",inputsLarger:"${inputsLarger}"}});fluid.defaults("fluid.uiOptions.selectDecorator",{gradeNames:["fluid.viewComponent","autoInit"],listeners:{onCreate:"fluid.uiOptions.selectDecorator.decorateOptions"},styles:{preview:"fl-preview-theme"}});fluid.uiOptions.selectDecorator.decorateOptions=function(that){fluid.each($("option",that.container),function(option){var styles=that.options.styles;$(option).addClass(styles.preview+" "+styles[fluid.value(option)])})}})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.uiOptions.modelRelay",{gradeNames:["fluid.modelComponent","fluid.eventedComponent","autoInit"],listenerNamespaces:[],mergePolicy:{sourceApplier:"nomerge"},listeners:{onCreate:"{that}.addListeners",onDestroy:"{that}.removeListeners"},invokers:{addListeners:{funcName:"fluid.uiOptions.modelRelay.addListeners",args:["{that}.options.rules","{that}.applier","{that}.options.sourceApplier","{that}.options.listenerNamespaces"]},removeListeners:{funcName:"fluid.uiOptions.modelRelay.removeListeners",args:["{that}.options.sourceApplier.modelChanged","{that}.options.listenerNamespaces"]}},sourceApplier:null,rules:{}});fluid.uiOptions.modelRelay.removeListeners=function(modelChanged,namespaces){fluid.each(namespaces,function(namespace){modelChanged.removeListener(namespace)})};fluid.uiOptions.modelRelay.addListeners=function(rules,applier,sourceApplier,listenerNamespaces){fluid.each(rules,function(internalKey,sourceKey){var uniqueNamespace=fluid.allocateGuid();listenerNamespaces.push(uniqueNamespace);fluid.addSourceGuardedListener(applier,internalKey,sourceKey,function(newModel){fluid.fireSourcedChange(sourceApplier,sourceKey,fluid.get(newModel,internalKey),internalKey)});fluid.addSourceGuardedListener(sourceApplier,sourceKey,internalKey,function(newModel){fluid.fireSourcedChange(applier,internalKey,fluid.get(newModel,sourceKey),sourceKey)},null,uniqueNamespace)})}})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.uiOptions.enactors",{gradeNames:["fluid.modelComponent","fluid.eventedComponent","fluid.uiOptions.modelRelay","autoInit"]});fluid.defaults("fluid.uiOptions.enactors.styleElements",{gradeNames:["fluid.uiOptions.enactors","autoInit"],cssClass:null,invokers:{applyStyle:{funcName:"fluid.uiOptions.enactors.styleElements.applyStyle",args:["{arguments}.0","{arguments}.1"]},resetStyle:{funcName:"fluid.uiOptions.enactors.styleElements.resetStyle",args:["{arguments}.0","{arguments}.1"]},handleStyle:{funcName:"fluid.uiOptions.enactors.styleElements.handleStyle",args:["{arguments}.0",{expander:{func:"{that}.getElements"}},"{that}"]},getElements:"fluid.uiOptions.enactors.getElements"},listeners:{onCreate:{listener:"{that}.handleStyle",args:["{that}.model.value"]}}});fluid.uiOptions.enactors.styleElements.applyStyle=function(elements,cssClass){elements.addClass(cssClass)};fluid.uiOptions.enactors.styleElements.resetStyle=function(elements,cssClass){$(elements,"."+cssClass).andSelf().removeClass(cssClass)};fluid.uiOptions.enactors.styleElements.handleStyle=function(value,elements,that){if(value){that.applyStyle(elements,that.options.cssClass)}else{that.resetStyle(elements,that.options.cssClass)}};fluid.uiOptions.enactors.styleElements.finalInit=function(that){that.applier.modelChanged.addListener("value",function(newModel){that.handleStyle(newModel.value)})};fluid.defaults("fluid.uiOptions.enactors.classSwapper",{gradeNames:["fluid.viewComponent","fluid.uiOptions.enactors","autoInit"],classes:{},invokers:{clearClasses:{funcName:"fluid.uiOptions.enactors.classSwapper.clearClasses",args:["{that}.container","{that}.classStr"]},swap:{funcName:"fluid.uiOptions.enactors.classSwapper.swap",args:["{arguments}.0","{that}"]}},listeners:{onCreate:{listener:"{that}.swap",args:["{that}.model.value"]}},members:{classStr:{expander:{func:"fluid.uiOptions.enactors.classSwapper.joinClassStr",args:"{that}.options.classes"}}}});fluid.uiOptions.enactors.classSwapper.clearClasses=function(container,classStr){container.removeClass(classStr)};fluid.uiOptions.enactors.classSwapper.swap=function(value,that){that.clearClasses();that.container.addClass(that.options.classes[value])};fluid.uiOptions.enactors.classSwapper.joinClassStr=function(classes){var classStr="";fluid.each(classes,function(oneClassName){if(oneClassName){classStr+=classStr?" "+oneClassName:oneClassName}});return classStr};fluid.uiOptions.enactors.classSwapper.finalInit=function(that){that.applier.modelChanged.addListener("value",function(newModel){that.swap(newModel.value)})};fluid.defaults("fluid.uiOptions.enactors.emphasizeLinks",{gradeNames:["fluid.viewComponent","fluid.uiOptions.enactors.styleElements","autoInit"],preferenceMap:{"fluid.uiOptions.emphasizeLinks":{"model.value":"default"}},cssClass:null,invokers:{getElements:{funcName:"fluid.uiOptions.enactors.emphasizeLinks.getLinks",args:"{that}.container"}}});fluid.uiOptions.enactors.emphasizeLinks.getLinks=function(container){return $("a",container)};fluid.defaults("fluid.uiOptions.enactors.inputsLarger",{gradeNames:["fluid.viewComponent","fluid.uiOptions.enactors.styleElements","autoInit"],preferenceMap:{"fluid.uiOptions.inputsLarger":{"model.value":"default"}},cssClass:null,invokers:{getElements:{funcName:"fluid.uiOptions.enactors.inputsLarger.getInputs",args:"{that}.container"}}});fluid.uiOptions.enactors.inputsLarger.getInputs=function(container){return $("input, button",container)};fluid.defaults("fluid.uiOptions.enactors.textFont",{gradeNames:["fluid.uiOptions.enactors.classSwapper","autoInit"],preferenceMap:{"fluid.uiOptions.textFont":{"model.value":"default"}}});fluid.defaults("fluid.uiOptions.enactors.contrast",{gradeNames:["fluid.uiOptions.enactors.classSwapper","autoInit"],preferenceMap:{"fluid.uiOptions.contrast":{"model.value":"default"}}});fluid.uiOptions.enactors.getTextSizeInPx=function(container,fontSizeMap){var fontSize=container.css("font-size");if(fontSizeMap[fontSize]){fontSize=fontSizeMap[fontSize]}return parseFloat(fontSize)};fluid.defaults("fluid.uiOptions.enactors.textSize",{gradeNames:["fluid.viewComponent","fluid.uiOptions.enactors","autoInit"],preferenceMap:{"fluid.uiOptions.textSize":{"model.value":"default"}},fontSizeMap:{},invokers:{set:{funcName:"fluid.uiOptions.enactors.textSize.set",args:["{arguments}.0","{that}"]},getTextSizeInPx:{funcName:"fluid.uiOptions.enactors.getTextSizeInPx",args:["{that}.container","{that}.options.fontSizeMap"]},getTextSizeInEm:{funcName:"fluid.uiOptions.enactors.textSize.getTextSizeInEm",args:[{expander:{func:"{that}.getTextSizeInPx"}},{expander:{func:"{that}.getPx2EmFactor"}}]},getPx2EmFactor:{funcName:"fluid.uiOptions.enactors.textSize.getPx2EmFactor",args:["{that}.container","{that}.options.fontSizeMap"]}},listeners:{onCreate:{listener:"{that}.set",args:"{that}.model.value"}}});fluid.uiOptions.enactors.textSize.set=function(times,that){if(!that.initialSize){that.initialSize=that.getTextSizeInEm()}if(that.initialSize){var targetSize=times*that.initialSize;that.container.css("font-size",targetSize+"em")}};fluid.uiOptions.enactors.textSize.getTextSizeInEm=function(textSizeInPx,px2emFactor){return Math.round(textSizeInPx/px2emFactor*10000)/10000};fluid.uiOptions.enactors.textSize.getPx2EmFactor=function(container,fontSizeMap){if(container.get(0).tagName!=="HTML"){container=container.parent()}return fluid.uiOptions.enactors.getTextSizeInPx(container,fontSizeMap)};fluid.uiOptions.enactors.textSize.finalInit=function(that){that.applier.modelChanged.addListener("value",function(newModel){that.set(newModel.value)})};fluid.defaults("fluid.uiOptions.enactors.lineSpace",{gradeNames:["fluid.viewComponent","fluid.uiOptions.enactors","autoInit"],preferenceMap:{"fluid.uiOptions.lineSpace":{"model.value":"default"}},fontSizeMap:{},invokers:{set:{funcName:"fluid.uiOptions.enactors.lineSpace.set",args:["{arguments}.0","{that}"]},getTextSizeInPx:{funcName:"fluid.uiOptions.enactors.getTextSizeInPx",args:["{that}.container","{that}.options.fontSizeMap"]},getLineHeight:{funcName:"fluid.uiOptions.enactors.lineSpace.getLineHeight",args:"{that}.container"},numerizeLineHeight:{funcName:"fluid.uiOptions.enactors.lineSpace.numerizeLineHeight",args:[{expander:{func:"{that}.getLineHeight"}},{expander:{func:"{that}.getTextSizeInPx"}}]}},listeners:{onCreate:{listener:"{that}.set",args:"{that}.model.value"}}});fluid.uiOptions.enactors.lineSpace.getLineHeight=function(container){var lineHeight;if(container[0].currentStyle){lineHeight=container[0].currentStyle.lineHeight}else{lineHeight=container.css("line-height")}return lineHeight};fluid.uiOptions.enactors.lineSpace.numerizeLineHeight=function(lineHeight,fontSize){if(!lineHeight){return 0}if(lineHeight==="normal"){return 1.2}if(lineHeight.match(/[0-9]$/)){return lineHeight}return Math.round(parseFloat(lineHeight)/fontSize*100)/100};fluid.uiOptions.enactors.lineSpace.set=function(times,that){if(!that.initialSize){that.initialSize=that.numerizeLineHeight()}if(that.initialSize){var targetLineSpace=times*that.initialSize;that.container.css("line-height",targetLineSpace)}};fluid.uiOptions.enactors.lineSpace.finalInit=function(that){that.applier.modelChanged.addListener("value",function(newModel){that.set(newModel.value)})};fluid.defaults("fluid.uiOptions.enactors.tableOfContents",{gradeNames:["fluid.viewComponent","fluid.uiOptions.enactors","autoInit"],preferenceMap:{"fluid.uiOptions.tableOfContents":{"model.value":"default"}},tocTemplate:null,components:{tableOfContents:{type:"fluid.tableOfContents",container:"{fluid.uiOptions.enactors.tableOfContents}.container",createOnEvent:"onCreateTOCReady",options:{components:{levels:{type:"fluid.tableOfContents.levels",options:{resources:{template:{forceCache:true,url:"{fluid.uiOptions.enactors.tableOfContents}.options.tocTemplate"}}}}},listeners:{afterRender:"{fluid.uiOptions.enactors.tableOfContents}.events.afterTocRender"}}}},invokers:{applyToc:{funcName:"fluid.uiOptions.enactors.tableOfContents.applyToc",args:["{arguments}.0","{that}"]}},events:{onCreateTOCReady:null,afterTocRender:null,onLateRefreshRelay:null},listeners:{onCreate:{listener:"{that}.applyToc",args:"{that}.model.value"}}});fluid.uiOptions.enactors.tableOfContents.applyToc=function(value,that){var async=false;if(value){if(that.tableOfContents){that.tableOfContents.show()}else{that.events.onCreateTOCReady.fire();async=true}}else{if(that.tableOfContents){that.tableOfContents.hide()}}if(!async){that.events.onLateRefreshRelay.fire(that)}};fluid.uiOptions.enactors.tableOfContents.finalInit=function(that){that.applier.modelChanged.addListener("value",function(newModel){that.applyToc(newModel.value)})};fluid.defaults("fluid.uiOptions.tocWithEmphasizeLinks",{gradeNames:["fluid.eventedComponent","autoInit"],listeners:{afterTocRender:{listener:"{uiEnhancer}.emphasizeLinks.handleStyle",args:"{uiEnhancer}.model.links"},onLateRefreshRelay:{listener:"{uiEnhancer}.emphasizeLinks.handleStyle",args:"{uiEnhancer}.model.links"}}});fluid.defaults("fluid.uiOptions.tocWithInputsLarger",{gradeNames:["fluid.eventedComponent","autoInit"],listeners:{afterTocRender:{listener:"{uiEnhancer}.inputsLarger.handleStyle",args:"{uiEnhancer}.model.inputsLarger"},onLateRefreshRelay:{listener:"{uiEnhancer}.inputsLarger.handleStyle",args:"{uiEnhancer}.model.inputsLarger"}}});fluid.demands("fluid.uiOptions.enactors.tableOfContents","fluid.uiOptions.enactors.emphasizeLinks",{options:{gradeNames:"fluid.uiOptions.tocWithEmphasizeLinks"}});fluid.demands("fluid.uiOptions.enactors.tableOfContents","fluid.uiOptions.enactors.inputsLarger",{options:{gradeNames:"fluid.uiOptions.tocWithInputsLarger"}})})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.uiOptions.rootModel.starter",{gradeNames:["fluid.uiOptions.rootModel","autoInit"],members:{rootModel:{textFont:"default",theme:"default",textSize:1,lineSpace:1,toc:false,links:false,inputsLarger:false}}});fluid.defaults("fluid.uiEnhancer.cssClassEnhancerBase",{gradeNames:["fluid.littleComponent","autoInit"],classnameMap:{textFont:{"default":"",times:"fl-font-uio-times",comic:"fl-font-uio-comic-sans",arial:"fl-font-uio-arial",verdana:"fl-font-uio-verdana"},theme:{"default":"fl-theme-uio-default",bw:"fl-theme-uio-bw fl-theme-bw",wb:"fl-theme-uio-wb fl-theme-wb",by:"fl-theme-uio-by fl-theme-by",yb:"fl-theme-uio-yb fl-theme-yb",lgdg:"fl-theme-uio-lgdg fl-theme-lgdg"},links:"fl-link-enhanced",inputsLarger:"fl-text-larger"}});fluid.defaults("fluid.uiEnhancer.browserTextEnhancerBase",{gradeNames:["fluid.littleComponent","autoInit"],fontSizeMap:{"xx-small":"9px","x-small":"11px",small:"13px",medium:"15px",large:"18px","x-large":"23px","xx-large":"30px"}});fluid.defaults("fluid.uiEnhancer.starterEnactors",{gradeNames:["fluid.uiEnhancer","fluid.uiEnhancer.cssClassEnhancerBase","fluid.uiEnhancer.browserTextEnhancerBase","autoInit"],components:{textSize:{type:"fluid.uiOptions.enactors.textSize",container:"{uiEnhancer}.container",options:{fontSizeMap:"{uiEnhancer}.options.fontSizeMap",sourceApplier:"{uiEnhancer}.applier",rules:{textSize:"value"},model:{value:"{fluid.uiOptions.rootModel}.rootModel.textSize"}}},textFont:{type:"fluid.uiOptions.enactors.textFont",container:"{uiEnhancer}.container",options:{classes:"{uiEnhancer}.options.classnameMap.textFont",sourceApplier:"{uiEnhancer}.applier",rules:{textFont:"value"},model:{value:"{fluid.uiOptions.rootModel}.rootModel.textFont"}}},lineSpace:{type:"fluid.uiOptions.enactors.lineSpace",container:"{uiEnhancer}.container",options:{fontSizeMap:"{uiEnhancer}.options.fontSizeMap",sourceApplier:"{uiEnhancer}.applier",rules:{lineSpace:"value"},model:{value:"{fluid.uiOptions.rootModel}.rootModel.lineSpace"}}},contrast:{type:"fluid.uiOptions.enactors.contrast",container:"{uiEnhancer}.container",options:{classes:"{uiEnhancer}.options.classnameMap.theme",sourceApplier:"{uiEnhancer}.applier",rules:{theme:"value"},model:{value:"{fluid.uiOptions.rootModel}.rootModel.theme"}}},emphasizeLinks:{type:"fluid.uiOptions.enactors.emphasizeLinks",container:"{uiEnhancer}.container",options:{cssClass:"{uiEnhancer}.options.classnameMap.links",sourceApplier:"{uiEnhancer}.applier",rules:{links:"value"},model:{links:"{fluid.uiOptions.rootModel}.rootModel.links"}}},inputsLarger:{type:"fluid.uiOptions.enactors.inputsLarger",container:"{uiEnhancer}.container",options:{cssClass:"{uiEnhancer}.options.classnameMap.inputsLarger",sourceApplier:"{uiEnhancer}.applier",rules:{inputsLarger:"value"},model:{inputsLarger:"{fluid.uiOptions.rootModel}.rootModel.inputsLarger"}}},tableOfContents:{type:"fluid.uiOptions.enactors.tableOfContents",container:"{uiEnhancer}.container",options:{tocTemplate:"{uiEnhancer}.options.tocTemplate",sourceApplier:"{uiEnhancer}.applier",rules:{toc:"value"},model:{toc:"{fluid.uiOptions.rootModel}.rootModel.toc"}}}}});fluid.defaults("fluid.uiOptions.defaultPanel",{gradeNames:["fluid.eventedComponent","autoInit"],mergePolicy:{sourceApplier:"nomerge"},sourceApplier:"{fluid.uiOptions}.applier",listeners:{"{uiOptions}.events.onUIOptionsRefresh":"{fluid.uiOptions.panels}.refreshView"},strings:{},parentBundle:"{uiOptionsLoader}.msgBundle"});fluid.defaults("fluid.uiOptions.starterPanels",{gradeNames:["fluid.uiOptions","autoInit"],selectors:{textSize:".flc-uiOptions-text-size",textFont:".flc-uiOptions-text-font",lineSpace:".flc-uiOptions-line-space",contrast:".flc-uiOptions-contrast",textControls:".flc-uiOptions-text-controls",layoutControls:".flc-uiOptions-layout-controls",linksControls:".flc-uiOptions-links-controls"},components:{textSize:{type:"fluid.uiOptions.panels.textSize",container:"{uiOptions}.dom.textSize",createOnEvent:"onUIOptionsMarkupReady",options:{gradeNames:"fluid.uiOptions.defaultPanel",rules:{textSize:"value"},model:{value:"{fluid.uiOptions.rootModel}.rootModel.textSize"},resources:{template:"{templateLoader}.resources.textSize"}}},lineSpace:{type:"fluid.uiOptions.panels.lineSpace",container:"{uiOptions}.dom.lineSpace",createOnEvent:"onUIOptionsMarkupReady",options:{gradeNames:"fluid.uiOptions.defaultPanel",rules:{lineSpace:"value"},model:{value:"{fluid.uiOptions.rootModel}.rootModel.lineSpace"},resources:{template:"{templateLoader}.resources.lineSpace"}}},textFont:{type:"fluid.uiOptions.panels.textFont",container:"{uiOptions}.dom.textFont",createOnEvent:"onUIOptionsMarkupReady",options:{gradeNames:"fluid.uiOptions.defaultPanel",classnameMap:"{uiEnhancer}.options.classnameMap",rules:{textFont:"value"},model:{value:"{fluid.uiOptions.rootModel}.rootModel.textFont"},resources:{template:"{templateLoader}.resources.textFont"}}},contrast:{type:"fluid.uiOptions.panels.contrast",container:"{uiOptions}.dom.contrast",createOnEvent:"onUIOptionsMarkupReady",options:{gradeNames:"fluid.uiOptions.defaultPanel",classnameMap:"{uiEnhancer}.options.classnameMap",rules:{theme:"value"},model:{value:"{fluid.uiOptions.rootModel}.rootModel.theme"},resources:{template:"{templateLoader}.resources.contrast"}}},layoutControls:{type:"fluid.uiOptions.panels.layoutControls",container:"{uiOptions}.dom.layoutControls",createOnEvent:"onUIOptionsMarkupReady",options:{gradeNames:"fluid.uiOptions.defaultPanel",rules:{toc:"toc"},model:{toc:"{fluid.uiOptions.rootModel}.rootModel.toc"},resources:{template:"{templateLoader}.resources.layoutControls"}}},linksControls:{type:"fluid.uiOptions.panels.linksControls",container:"{uiOptions}.dom.linksControls",createOnEvent:"onUIOptionsMarkupReady",options:{gradeNames:"fluid.uiOptions.defaultPanel",rules:{links:"links",inputsLarger:"inputsLarger"},model:{links:"{fluid.uiOptions.rootModel}.rootModel.links",inputsLarger:"{fluid.uiOptions.rootModel}.rootModel.inputsLarger"},resources:{template:"{templateLoader}.resources.linksControls"}}}}});fluid.defaults("fluid.uiOptions.starterTemplateLoader",{gradeNames:["fluid.uiOptions.resourceLoader","autoInit"],templates:{textSize:"%prefix/UIOptionsTemplate-textSize.html",textFont:"%prefix/UIOptionsTemplate-textFont.html",lineSpace:"%prefix/UIOptionsTemplate-lineSpace.html",contrast:"%prefix/UIOptionsTemplate-contrast.html",layoutControls:"%prefix/UIOptionsTemplate-layout.html",linksControls:"%prefix/UIOptionsTemplate-links.html"}});fluid.defaults("fluid.uiOptions.starterFatPanelTemplateLoader",{gradeNames:["fluid.uiOptions.starterTemplateLoader","autoInit"],templates:{uiOptions:"%prefix/FatPanelUIOptions.html"}});fluid.defaults("fluid.uiOptions.starterFullPreviewTemplateLoader",{gradeNames:["fluid.uiOptions.starterTemplateLoader","autoInit"],templates:{uiOptions:"%prefix/FullPreviewUIOptions.html"}});fluid.defaults("fluid.uiOptions.starterFullNoPreviewTemplateLoader",{gradeNames:["fluid.uiOptions.starterTemplateLoader","autoInit"],templates:{uiOptions:"%prefix/FullNoPreviewUIOptions.html"}});fluid.defaults("fluid.uiOptions.starterMessageLoader",{gradeNames:["fluid.uiOptions.resourceLoader","autoInit"],templates:{uiOptions:"%prefix/uiOptions.json",textSize:"%prefix/textSize.json",textFont:"%prefix/textFont.json",lineSpace:"%prefix/lineSpace.json",contrast:"%prefix/contrast.json",layoutControls:"%prefix/tableOfContents.json",linksControls:"%prefix/links.json"}})})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.registerNamespace("fluid.dom");fluid.dom.getDocumentHeight=function(dokkument){var body=$("body",dokkument)[0];return body.offsetHeight};fluid.defaults("fluid.uiOptions.fatPanel",{gradeNames:["fluid.uiOptions.uiOptionsLoader","autoInit"],events:{afterRender:null,onReady:null,onCreateSlidingPanelReady:{events:{iframeRendered:"afterRender",onMsgBundleReady:"onMsgBundleReady"}},templatesAndIframeReady:{events:{iframeReady:"afterRender",templatesLoaded:"onUIOTemplatesLoaded",messagesLoaded:"onUIOMessagesLoaded"}}},listeners:{onReady:{listener:"fluid.uiOptions.fatPanel.bindEvents",args:["{fatPanel}.uiOptions","{iframeRenderer}.iframeEnhancer","{fatPanel}"]},onCreate:{listener:"fluid.uiOptions.fatPanel.hideReset",args:["{fatPanel}"]}},selectors:{reset:".flc-uiOptions-reset",iframe:".flc-uiOptions-iframe"},invokers:{bindReset:{funcName:"fluid.bind",args:["{fatPanel}.dom.reset","click","{arguments}.0"]}},components:{pageEnhancer:"{uiEnhancer}",slidingPanel:{type:"fluid.slidingPanel",container:"{fatPanel}.container",createOnEvent:"onCreateSlidingPanelReady",options:{members:{msgBundle:"{fatPanel}.msgBundle"},invokers:{operateShow:{funcName:"fluid.uiOptions.fatPanel.showPanel"},operateHide:{funcName:"fluid.uiOptions.fatPanel.hidePanel"}}}},iframeRenderer:{type:"fluid.uiOptions.fatPanel.renderIframe",container:"{fatPanel}.dom.iframe",options:{markupProps:{src:"%templatePrefix/FatPanelUIOptionsFrame.html"},events:{afterRender:"{fatPanel}.events.afterRender"},components:{iframeEnhancer:{type:"fluid.uiEnhancer",container:"{iframeRenderer}.renderUIOContainer",createOnEvent:"afterRender",options:{gradeNames:["{pageEnhancer}.options.gradeNames"],jQuery:"{iframeRenderer}.jQuery",tocTemplate:"{pageEnhancer}.options.tocTemplate",events:{onIframeVisible:null}}}}}},uiOptions:{createOnEvent:"templatesAndIframeReady",container:"{iframeRenderer}.renderUIOContainer",options:{gradeNames:["fluid.uiOptions.uiEnhancerRelay"],model:"{fatPanel}.model",applier:"{fatPanel}.applier",events:{onSignificantDOMChange:null,updateEnhancerModel:"{that}.events.modelChanged"},listeners:{modelChanged:"{that}.save",onCreate:{listener:"{fatPanel}.bindReset",args:["{that}.reset"]},onReset:"{that}.applyChanges",onReady:"{fatPanel}.events.onReady"}}}},outerEnhancerOptions:"{originalEnhancerOptions}.options.originalUserOptions",distributeOptions:[{source:"{that}.options.slidingPanel",removeSource:true,target:"{that > slidingPanel}.options"},{source:"{that}.options.iframeRenderer",removeSource:true,target:"{that > iframeRenderer}.options"},{source:"{that}.options.iframe",removeSource:true,target:"{that}.options.selectors.iframe"},{source:"{that}.options.outerEnhancerOptions",removeSource:true,target:"{that iframeEnhancer}.options"},{source:"{that}.options.templatePrefix",target:"{that > iframeRenderer}.options.templatePrefix"}]});fluid.uiOptions.fatPanel.hideReset=function(fatPanel){fatPanel.locate("reset").hide()};fluid.defaults("fluid.uiOptions.fatPanel.renderIframe",{gradeNames:["fluid.viewComponent","autoInit"],events:{afterRender:null},styles:{containerFlex:"fl-container-flex",container:"fl-uiOptions-fatPanel-iframe"},templatePrefix:"./",markupProps:{"class":"flc-iframe",src:"%templatePrefix/uiOptionsIframe.html"}});fluid.uiOptions.fatPanel.renderIframe.finalInit=function(that){var styles=that.options.styles;that.options.markupProps.src=fluid.stringTemplate(that.options.markupProps.src,{"templatePrefix/":that.options.templatePrefix});that.iframeSrc=that.options.markupProps.src;that.iframe=$("<iframe/>");that.iframe.load(function(){var iframeWindow=that.iframe[0].contentWindow;that.iframeDocument=iframeWindow.document;that.jQuery=iframeWindow.jQuery;that.renderUIOContainer=that.jQuery("body",that.iframeDocument);that.jQuery(that.iframeDocument).ready(that.events.afterRender.fire)});that.iframe.attr(that.options.markupProps);that.iframe.addClass(styles.containerFlex);that.iframe.addClass(styles.container);that.iframe.hide();that.iframe.appendTo(that.container)};fluid.uiOptions.fatPanel.updateView=function(uiOptions){uiOptions.events.onUIOptionsRefresh.fire();uiOptions.events.onSignificantDOMChange.fire()};fluid.uiOptions.fatPanel.bindEvents=function(uiOptions,iframeEnhancer,fatPanel){fatPanel.slidingPanel.events.afterPanelShow.addListener(function(){iframeEnhancer.events.onIframeVisible.fire(iframeEnhancer);fluid.uiOptions.fatPanel.updateView(uiOptions)});uiOptions.events.onUIOptionsRefresh.addListener(function(){iframeEnhancer.updateModel(uiOptions.model)});uiOptions.events.onReset.addListener(function(uiOptions){fluid.uiOptions.fatPanel.updateView(uiOptions)});uiOptions.events.onSignificantDOMChange.addListener(function(){var dokkument=uiOptions.container[0].ownerDocument;var height=fluid.dom.getDocumentHeight(dokkument);var iframe=fatPanel.iframeRenderer.iframe;var attrs={height:height+15};var panel=fatPanel.slidingPanel.locate("panel");panel.css({height:""});iframe.animate(attrs,400)});iframeEnhancer.events.onIframeVisible.addListener(function(){iframeEnhancer.applier.requestChange("",iframeEnhancer.model)});fatPanel.slidingPanel.events.afterPanelHide.addListener(function(){fatPanel.iframeRenderer.iframe.height(0);fatPanel.iframeRenderer.iframe.hide()});fatPanel.slidingPanel.events.onPanelShow.addListener(function(){fatPanel.iframeRenderer.iframe.show();fatPanel.locate("reset").show()});fatPanel.slidingPanel.events.onPanelHide.addListener(function(){fatPanel.locate("reset").hide()})};fluid.uiOptions.fatPanel.hidePanel=function(panel,callback){$(panel).animate({height:0},{duration:400,complete:callback})};fluid.uiOptions.fatPanel.showPanel=function(panel,callback){setTimeout(callback,1)}})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.uiOptions.fullNoPreview",{gradeNames:["fluid.uiOptions.uiOptionsLoader","autoInit"],components:{uiOptions:{container:"{that}.container",options:{listeners:{onReset:[{listener:"{that}.applyChanges"},{listener:"{that}.save"}]}}}}})})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.uiOptions.fullPreview",{gradeNames:["fluid.uiOptions.uiOptionsLoader","autoInit"],outerPreviewEnhancerOptions:"{originalEnhancerOptions}.options.originalUserOptions",components:{uiOptions:{container:"{that}.container",options:{components:{preview:{type:"fluid.uiOptions.preview",createOnEvent:"onReady",container:"{uiOptions}.dom.previewFrame"}},distributeOptions:{source:"{that}.options.preview",removeSource:true,target:"{that > preview}.options"}}}},distributeOptions:[{source:"{that}.options.outerPreviewEnhancerOptions",removeSource:true,target:"{that enhancer}.options"},{source:"{that}.options.preview",target:"{that preview}.options"},{source:"{that}.options.previewEnhancer",removeSource:true,target:"{that enhancer}.options"}]})})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.defaults("fluid.slidingPanel",{gradeNames:["fluid.viewComponent","autoInit"],selectors:{panel:".flc-slidingPanel-panel",toggleButton:".flc-slidingPanel-toggleButton"},strings:{showText:{expander:{func:"fluid.slidingPanel.lookupMsg",args:["{that}.msgBundle","slidingPanelShowText"]}},hideText:{expander:{func:"fluid.slidingPanel.lookupMsg",args:["{that}.msgBundle","slidingPanelHideText"]}}},events:{onPanelHide:null,onPanelShow:null,afterPanelHide:null,afterPanelShow:null},finalInitFunction:"fluid.slidingPanel.finalInit",invokers:{operateHide:"fluid.slidingPanel.slideUp",operateShow:"fluid.slidingPanel.slideDown"},model:{isShowing:false},methods:{showPanel:{finalState:true,name:"Show"},hidePanel:{finalState:false,name:"Hide"}}});fluid.slidingPanel.lookupMsg=function(messageResolver,value){var looked=messageResolver.lookup([value]);return looked?looked.template:looked};fluid.slidingPanel.slideUp=function(element,callback,duration){$(element).slideUp(duration||"400",callback)};fluid.slidingPanel.slideDown=function(element,callback,duration){$(element).slideDown(duration||"400",callback)};fluid.slidingPanel.finalInit=function(that){fluid.each(that.options.methods,function(method,methodName){that[methodName]=function(){that.events["onPanel"+method.name].fire(that);that.applier.requestChange("isShowing",method.finalState);that.refreshView();that["operate"+method.name](that.locate("panel"),that.events["afterPanel"+method.name].fire)}});that.togglePanel=function(){that[that.model.isShowing?"hidePanel":"showPanel"]()};that.setPanelHeight=function(newHeight){that.locate("panel").height(newHeight)};that.refreshView=function(){that.locate("toggleButton").text(that.options.strings[that.model.isShowing?"hideText":"showText"])};that.locate("toggleButton").click(that.togglePanel);that.refreshView()}})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.registerNamespace("fluid.uiOptions.schemas");fluid.uiOptions.schemas.merge=function(target,source){if(!target){target={type:"object",properties:{}}}source=source.properties||source;$.extend(true,target.properties,source);return target};fluid.defaults("fluid.uiOptions.primaryBuilder",{gradeNames:["fluid.littleComponent","autoInit","{that}.buildPrimary"],schemaIndex:{expander:{func:"fluid.indexDefaults",args:["schemaIndex",{gradeNames:"fluid.uiOptions.schemas",indexFunc:"fluid.uiOptions.primaryBuilder.defaultSchemaIndexer"}]}},primarySchema:{},typeFilter:[],invokers:{buildPrimary:{funcName:"fluid.uiOptions.primaryBuilder.buildPrimary",args:["{that}.options.schemaIndex","{that}.options.typeFilter","{that}.options.primarySchema"]}}});fluid.uiOptions.primaryBuilder.buildPrimary=function(schemaIndex,typeFilter,primarySchema){var suppliedPrimaryGradeName="fluid.uiOptions.schemas.suppliedPrimary"+fluid.allocateGuid();fluid.defaults(suppliedPrimaryGradeName,{gradeNames:["autoInit","fluid.uiOptions.schemas"],schema:fluid.filterKeys(primarySchema.properties||primarySchema,typeFilter,false)});var primary=[suppliedPrimaryGradeName];fluid.each(typeFilter,function merge(type){var schemaGrades=schemaIndex[type];if(schemaGrades){primary.push.apply(primary,schemaGrades)}});return primary};fluid.uiOptions.primaryBuilder.defaultSchemaIndexer=function(defaults){if(defaults.schema){return fluid.keys(defaults.schema.properties)}};fluid.defaults("fluid.uiOptions.schemas",{gradeNames:["autoInit","fluid.littleComponent"],mergePolicy:{schema:fluid.uiOptions.schemas.merge}})})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.registerNamespace("fluid.uiOptions");fluid.defaults("fluid.uiOptions.auxSchema",{gradeNames:["fluid.littleComponent","autoInit"],auxiliarySchema:{}});fluid.uiOptions.expandSchemaValue=function(root,pathRef){if(pathRef.charAt(0)!=="@"){return pathRef}return fluid.get(root,pathRef.substring(1))};fluid.uiOptions.addAtPath=function(root,path,object){var existingObject=fluid.get(root,path);fluid.set(root,path,$.extend(true,{},existingObject,object));return root};fluid.uiOptions.removeKey=function(root,key){var value=root[key];delete root[key];return value};fluid.uiOptions.rearrangeDirect=function(root,toPath,sourcePath){var result={};var sourceValue=fluid.uiOptions.removeKey(root,sourcePath);if(sourceValue){fluid.set(result,toPath,sourceValue)}return result};fluid.uiOptions.expandSchemaComponents=function(auxSchema,type,prefKey,componentConfig,index,commonOptions,mappedDefaults){var componentOptions=fluid.copy(componentConfig)||{};var components={};var rootModel={};var componentName=fluid.uiOptions.removeKey(componentOptions,"type");var regexp=new RegExp("\\.","g");var memberName=componentName.replace(regexp,"_");var flattenedPrefKey=prefKey.replace(regexp,"_");if(componentName){var cmp=components[memberName]={type:componentName,options:componentOptions};var selectors=fluid.uiOptions.rearrangeDirect(componentOptions,memberName,"container");var templates=fluid.uiOptions.rearrangeDirect(componentOptions,memberName,"template");var messages=fluid.uiOptions.rearrangeDirect(componentOptions,memberName,"message");var preferenceMap=fluid.defaults(componentName).preferenceMap;var map=preferenceMap[prefKey];fluid.each(map,function(PrimaryPath,internalPath){var prefSchema=mappedDefaults[prefKey];if(prefSchema){var opts={};if(internalPath.indexOf("model.")===0){var internalModelName=internalPath.slice(6);fluid.set(opts,["rules",flattenedPrefKey],internalModelName);fluid.set(opts,["model",internalModelName],prefSchema[PrimaryPath]);fluid.set(rootModel,["members","rootModel",flattenedPrefKey],prefSchema[PrimaryPath])}else{fluid.set(opts,internalPath,prefSchema[PrimaryPath])}$.extend(true,componentOptions,opts)}});fluid.each(commonOptions,function(value,path){var opts={};value=fluid.stringTemplate(value,{prefKey:memberName});fluid.set(opts,path,value);$.extend(true,cmp,opts)});fluid.uiOptions.addAtPath(auxSchema,[type,"components"],components);fluid.uiOptions.addAtPath(auxSchema,[type,"selectors"],selectors);fluid.uiOptions.addAtPath(auxSchema,["templateLoader","templates"],templates);fluid.uiOptions.addAtPath(auxSchema,["messageLoader","templates"],messages);fluid.uiOptions.addAtPath(auxSchema,"rootModel",rootModel)}return auxSchema};fluid.uiOptions.expandSchemaDirectOption=function(auxSchema,type,targetPath){var value=auxSchema[type];if(value){delete auxSchema[type];fluid.set(auxSchema,targetPath,value)}};fluid.uiOptions.expandSchemaImpl=function(schemaToExpand,altSource){var expandedSchema=fluid.copy(schemaToExpand);altSource=altSource||expandedSchema;fluid.each(expandedSchema,function(value,key){if(typeof value==="object"){expandedSchema[key]=fluid.uiOptions.expandSchemaImpl(value,altSource)}else{if(typeof value==="string"){var expandedVal=fluid.uiOptions.expandSchemaValue(altSource,value);if(expandedVal!==undefined){expandedSchema[key]=expandedVal}else{delete expandedSchema[key]}}}});return expandedSchema};fluid.uiOptions.expandSchema=function(schemaToExpand,defaultNamespace,indexes,topCommonOptions,elementCommonOptions,mappedDefaults){var auxSchema=fluid.uiOptions.expandSchemaImpl(schemaToExpand);auxSchema.namespace=auxSchema.namespace||defaultNamespace;fluid.each(auxSchema,function(category,prefName){var type="panel";if(category[type]){fluid.uiOptions.expandSchemaComponents(auxSchema,"panels",category.type,category[type],fluid.get(indexes,type),fluid.get(elementCommonOptions,type),mappedDefaults)}type="enactor";if(category[type]){fluid.uiOptions.expandSchemaComponents(auxSchema,"enactors",category.type,category[type],fluid.get(indexes,type),fluid.get(elementCommonOptions,type),mappedDefaults)}type="template";if(prefName===type){fluid.set(auxSchema,["templateLoader","templates","uiOptions"],auxSchema[type]);delete auxSchema[type]}type="templatePrefix";if(prefName===type){fluid.uiOptions.expandSchemaDirectOption(auxSchema,type,"templatePrefix.templatePrefix")}type="message";if(prefName===type){fluid.set(auxSchema,["messageLoader","templates","uiOptions"],auxSchema[type]);delete auxSchema[type]}type="messagePrefix";if(prefName===type){fluid.uiOptions.expandSchemaDirectOption(auxSchema,type,"messagePrefix.messagePrefix")}});fluid.each(topCommonOptions,function(topOptions,type){var typeObject=fluid.get(auxSchema,type);if(typeObject){auxSchema[type]=$.extend(true,topOptions,typeObject)}});return auxSchema};fluid.defaults("fluid.uiOptions.auxBuilder",{gradeNames:["fluid.uiOptions.auxSchema","autoInit"],defaultNamespace:"fluid.uiOptions.create",mergePolicy:{elementCommonOptions:"noexpand"},topCommonOptions:{panels:{gradeNames:["fluid.uiOptions","autoInit"]},enactors:{gradeNames:["fluid.uiEnhancer","autoInit"]},templateLoader:{gradeNames:["fluid.uiOptions.resourceLoader","autoInit"]},messageLoader:{gradeNames:["fluid.uiOptions.resourceLoader","autoInit"]},rootModel:{gradeNames:["fluid.uiOptions.rootModel","autoInit"]},templatePrefix:{gradeNames:["fluid.littleComponent","autoInit"]},messagePrefix:{gradeNames:["fluid.littleComponent","autoInit"]}},elementCommonOptions:{panel:{createOnEvent:"onUIOptionsMarkupReady",container:"{uiOptions}.dom.%prefKey","options.gradeNames":"fluid.uiOptions.defaultPanel","options.resources.template":"{templateLoader}.resources.%prefKey"},enactor:{container:"{uiEnhancer}.container","options.sourceApplier":"{uiEnhancer}.applier"}},indexes:{panel:{expander:{func:"fluid.indexDefaults",args:["panelsIndex",{gradeNames:"fluid.uiOptions.panels",indexFunc:"fluid.uiOptions.auxBuilder.prefMapIndexer"}]}},enactor:{expander:{func:"fluid.indexDefaults",args:["enactorsIndex",{gradeNames:"fluid.uiOptions.enactors",indexFunc:"fluid.uiOptions.auxBuilder.prefMapIndexer"}]}}},mappedDefaults:{},expandedAuxSchema:{expander:{func:"fluid.uiOptions.expandSchema",args:["{that}.options.auxiliarySchema","{that}.options.defaultNamespace","{that}.options.indexes","{that}.options.topCommonOptions","{that}.options.elementCommonOptions","{that}.options.mappedDefaults"]}}});fluid.uiOptions.auxBuilder.prefMapIndexer=function(defaults){return fluid.keys(defaults.preferenceMap)}})(jQuery,fluid_1_5);var fluid_1_5=fluid_1_5||{};(function(fluid){fluid.defaults("fluid.uiOptions.auxSchema.starter",{gradeNames:["fluid.uiOptions.auxSchema","autoInit"],auxiliarySchema:{namespace:"fluid.uiOptions.constructed",templatePrefix:"../../../components/uiOptions/html/",template:"%prefix/FatPanelUIOptions.html",messagePrefix:"../../../components/uiOptions/messages/",message:"%prefix/uiOptions.json",textSize:{type:"fluid.uiOptions.textSize",enactor:{type:"fluid.uiOptions.enactors.textSize"},panel:{type:"fluid.uiOptions.panels.textSize",container:".flc-uiOptions-text-size",template:"%prefix/UIOptionsTemplate-textSize.html",message:"%prefix/textSize.json"}},lineSpace:{type:"fluid.uiOptions.lineSpace",enactor:{type:"fluid.uiOptions.enactors.lineSpace",fontSizeMap:{"xx-small":"9px","x-small":"11px",small:"13px",medium:"15px",large:"18px","x-large":"23px","xx-large":"30px"}},panel:{type:"fluid.uiOptions.panels.lineSpace",container:".flc-uiOptions-line-space",template:"%prefix/UIOptionsTemplate-lineSpace.html",message:"%prefix/lineSpace.json"}},textFont:{type:"fluid.uiOptions.textFont",classes:{"default":"",times:"fl-font-uio-times",comic:"fl-font-uio-comic-sans",arial:"fl-font-uio-arial",verdana:"fl-font-uio-verdana"},enactor:{type:"fluid.uiOptions.enactors.textFont",classes:"@textFont.classes"},panel:{type:"fluid.uiOptions.panels.textFont",container:".flc-uiOptions-text-font",classnameMap:{textFont:"@textFont.classes"},template:"%prefix/UIOptionsTemplate-textFont.html",message:"%prefix/textFont.json"}},contrast:{type:"fluid.uiOptions.contrast",classes:{"default":"fl-theme-uio-default",bw:"fl-theme-uio-bw fl-theme-bw",wb:"fl-theme-uio-wb fl-theme-wb",by:"fl-theme-uio-by fl-theme-by",yb:"fl-theme-uio-yb fl-theme-yb",lgdg:"fl-theme-uio-lgdg fl-theme-lgdg"},enactor:{type:"fluid.uiOptions.enactors.contrast",classes:"@contrast.classes"},panel:{type:"fluid.uiOptions.panels.contrast",container:".flc-uiOptions-contrast",classnameMap:{theme:"@contrast.classes"},template:"%prefix/UIOptionsTemplate-contrast.html",message:"%prefix/contrast.json"}},tableOfContents:{type:"fluid.uiOptions.tableOfContents",enactor:{type:"fluid.uiOptions.enactors.tableOfContents",tocTemplate:"../../../../components/tableOfContents/html/TableOfContents.html"},panel:{type:"fluid.uiOptions.panels.layoutControls",container:".flc-uiOptions-layout-controls",template:"%prefix/UIOptionsTemplate-layout.html",message:"%prefix/tableOfContents.json"}},emphasizeLinks:{type:"fluid.uiOptions.emphasizeLinks",enactor:{type:"fluid.uiOptions.enactors.emphasizeLinks",cssClass:"fl-link-enhanced"},panel:{type:"fluid.uiOptions.panels.linksControls",container:".flc-uiOptions-links-controls",template:"%prefix/UIOptionsTemplate-links.html",message:"%prefix/links.json"}},inputsLarger:{type:"fluid.uiOptions.inputsLarger",enactor:{type:"fluid.uiOptions.enactors.inputsLarger",cssClass:"fl-text-larger"},panel:{type:"fluid.uiOptions.panels.linksControls"}}}});fluid.defaults("fluid.uiOptions.schemas.textSize",{gradeNames:["autoInit","fluid.uiOptions.schemas"],schema:{"fluid.uiOptions.textSize":{type:"number","default":1,minimum:1,maximum:2,divisibleBy:0.1}}});fluid.defaults("fluid.uiOptions.schemas.lineSpace",{gradeNames:["autoInit","fluid.uiOptions.schemas"],schema:{"fluid.uiOptions.lineSpace":{type:"number","default":1,minimum:1,maximum:2,divisibleBy:0.1}}});fluid.defaults("fluid.uiOptions.schemas.textFont",{gradeNames:["autoInit","fluid.uiOptions.schemas"],schema:{"fluid.uiOptions.textFont":{type:"string","default":"","enum":["default","times","comic","arial","verdana"]}}});fluid.defaults("fluid.uiOptions.schemas.contrast",{gradeNames:["autoInit","fluid.uiOptions.schemas"],schema:{"fluid.uiOptions.contrast":{type:"string","default":"default","enum":["default","bw","wb","by","yb","lgdg"]}}});fluid.defaults("fluid.uiOptions.schemas.tableOfContents",{gradeNames:["autoInit","fluid.uiOptions.schemas"],schema:{"fluid.uiOptions.tableOfContents":{type:"boolean","default":false}}});fluid.defaults("fluid.uiOptions.schemas.emphasizeLinks",{gradeNames:["autoInit","fluid.uiOptions.schemas"],schema:{"fluid.uiOptions.emphasizeLinks":{type:"boolean","default":false}}});fluid.defaults("fluid.uiOptions.schemas.inputsLarger",{gradeNames:["autoInit","fluid.uiOptions.schemas"],schema:{"fluid.uiOptions.inputsLarger":{type:"boolean","default":false}}})})(fluid_1_5);var fluid_1_5=fluid_1_5||{};(function($,fluid){fluid.registerNamespace("fluid.uiOptions");fluid.defaults("fluid.uiOptions.builder",{gradeNames:["fluid.eventedComponent","fluid.uiOptions.auxBuilder","autoInit"],mergePolicy:{auxSchema:"expandedAuxSchema"},assembledUIOGrade:{expander:{func:"fluid.uiOptions.builder.generateGrade",args:["uio","{that}.options.auxSchema.namespace",{gradeNames:["fluid.viewComponent","autoInit","fluid.uiOptions.assembler.uio"],componentGrades:"{that}.options.constructedGrades"}]}},assembledUIEGrade:{expander:{func:"fluid.uiOptions.builder.generateGrade",args:["uie","{that}.options.auxSchema.namespace",{gradeNames:["fluid.viewComponent","autoInit","fluid.uiOptions.assembler.uie"],componentGrades:"{that}.options.constructedGrades"}]}},constructedGrades:{expander:{func:"fluid.uiOptions.builder.constructGrades",args:["{that}.options.auxSchema",["enactors","messages","panels","rootModel","templateLoader","messageLoader","templatePrefix","messagePrefix"]]}},mappedDefaults:"{primaryBuilder}.options.schema.properties",components:{primaryBuilder:{type:"fluid.uiOptions.primaryBuilder",options:{typeFilter:{expander:{func:"fluid.uiOptions.builder.parseAuxSchema",args:"{builder}.options.auxiliarySchema"}},}}},distributeOptions:[{source:"{that}.options.primarySchema",removeSource:true,target:"{that > primaryBuilder}.options.primarySchema"}]});fluid.defaults("fluid.uiOptions.assembler.uie",{gradeNames:["autoInit","fluid.viewComponent"],components:{store:{type:"fluid.globalSettingsStore"},enhancer:{type:"fluid.littleComponent",options:{gradeNames:"{that}.options.enhancerType",enhancerType:"fluid.pageEnhancer",components:{uiEnhancer:{options:{gradeNames:["{fluid.uiOptions.assembler.uie}.options.componentGrades.enactors"]}}}}}},distributeOptions:[{source:"{that}.options.enhancer",removeSource:true,target:"{that uiEnhancer}.options"}]});fluid.defaults("fluid.uiOptions.assembler.uio",{gradeNames:["autoInit","fluid.viewComponent","fluid.uiOptions.assembler.uie"],components:{uiOptionsLoader:{type:"fluid.viewComponent",container:"{fluid.uiOptions.assembler.uio}.container",priority:"last",options:{gradeNames:["{fluid.uiOptions.assembler.uio}.options.componentGrades.templatePrefix","{fluid.uiOptions.assembler.uio}.options.componentGrades.messagePrefix","{fluid.uiOptions.assembler.uio}.options.componentGrades.messages","{that}.options.uioType"],uioType:"fluid.uiOptions.fatPanel",templateLoader:{gradeNames:["{fluid.uiOptions.assembler.uio}.options.componentGrades.templateLoader"]},messageLoader:{gradeNames:["{fluid.uiOptions.assembler.uio}.options.componentGrades.messageLoader"]},uiOptions:{gradeNames:["{fluid.uiOptions.assembler.uio}.options.componentGrades.panels","{fluid.uiOptions.assembler.uio}.options.componentGrades.rootModel","fluid.uiOptions.uiEnhancerRelay"]}}}},distributeOptions:[{source:"{that}.options.uioType",removeSource:true,target:"{that > uiOptionsLoader}.options.uioType"},{source:"{that}.options.uiOptions",removeSource:true,target:"{that uiOptions}.options"}]});fluid.uiOptions.builder.generateGrade=function(name,namespace,options){var gradeNameTemplate="%namespace.%name";var gradeName=fluid.stringTemplate(gradeNameTemplate,{name:name,namespace:namespace});fluid.defaults(gradeName,options);return gradeName};fluid.uiOptions.builder.constructGrades=function(auxSchema,gradeCategories){var constructedGrades={};fluid.each(gradeCategories,function(category){var gradeOpts=auxSchema[category];if(fluid.get(gradeOpts,"gradeNames")){constructedGrades[category]=fluid.uiOptions.builder.generateGrade(category,auxSchema.namespace,gradeOpts)}});return constructedGrades};fluid.uiOptions.builder.parseAuxSchema=function(auxSchema){var auxTypes=[];fluid.each(auxSchema,function parse(field){var type=field.type;if(type){auxTypes.push(type)}});return auxTypes}})(jQuery,fluid_1_5);